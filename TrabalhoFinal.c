...
void remover_vertice(Grafo* g, const char* nome) {
    ...
    // Remover arestas que apontam para esse vértice
    for (int i = 0; i < g->num_vertices; i++) {
        ...
        while (*atual != NULL) {
            if ((*atual)->destino == indice) {
                ...
                *atual = (*atual)->prox;
                ...
                // Se o destino for maior que o índice removido, decrementa
                if ((*atual)->destino > indice) {
                    (*atual)->destino--;
                }
                atual = &(*atual)->prox;
    ...
    // Liberar a lista de arestas do próprio vértice
    ...
    // Mover os vértices para fechar o buraco
    for (int i = indice; i < g->num_vertices - 1; i++) {
        g->vertices[i] = g->vertices[i + 1];
    }

    // Atualizar as arestas de todos os vértices após a movimentação
    for (int i = 0; i < g->num_vertices - 1; i++) {
        Aresta* a = g->vertices[i].lista_arestas;
        while (a != NULL) {
            if (a->destino > indice)
                a->destino--;  // deslocamento para manter consistência
            a = a->prox;
        }
    }
